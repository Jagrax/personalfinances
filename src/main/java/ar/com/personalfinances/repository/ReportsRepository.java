...
public interface ReportsRepository extends JpaRepository<User, Long> {
    ...
    @Query("SELECT a.name, a.currency, sum(e.amount), CASE WHEN a.type = 'BANK_ACCOUNT' THEN b.logo ELSE CASE WHEN LOWER(REPLACE(a.name, ' ', '')) = 'visa' THEN 'visa.svg' WHEN LOWER(REPLACE(a.name, ' ', '')) = 'mastercard' THEN 'mastercard.svg' WHEN LOWER(REPLACE(a.name, ' ', '')) = 'mercadopago' THEN 'mercadopago.svg' WHEN LOWER(REPLACE(a.name, ' ', '')) = 'sdd' THEN 'sdd.png' ELSE '' END END, a.syncEnabled FROM Expense e, Account a, User u LEFT JOIN a.bank b WHERE u.id = :userId AND a.owner = u AND e.account = a GROUP BY a.id, a.currency, a.syncEnabled")
